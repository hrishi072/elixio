...
def follow_user(request, user_id):
    ...
    if request.user in user.profile.followers.all():
        ...
        Notification.objects.create(
            Actor=request.user, Target=user, notif_type="follow"
        )
        text = "Unfollow"
...
def change_picture(request):
    ...
    if request.method == "POST":
        ...
        profile_form = ProfileEditForm(instance=request.user.profile)
        return render(
            request, "users/change_picture.html", {"profile_form": profile_form}
        )
...
def send_message_request(request, user_id):
    ...
    if contacter in receiver.profile.pending_list.all():
        ...
        Notification.objects.create(
            Actor=contacter, Target=receiver, notif_type="sent_msg_request"
        )
        text = "Request Sent"
    return HttpResponse(text)
...
def accept_message_request(request, user_id):
    ...
    if sender in acceptor.profile.pending_list.all():
        ...
        Notification.objects.create(
            Actor=acceptor, Target=sender, notif_type="confirmed_msg_request"
        )
...
def all_message_requests(request):
    ...
    return render(
        request,
        "users/view_all_message_requests.html",
        {"users": users, "page": page, "p": p, "p_obj": p_obj},
    )
